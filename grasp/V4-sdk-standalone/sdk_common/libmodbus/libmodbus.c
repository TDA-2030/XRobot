...
void modbus_set_inspector(modbus_t* ctx, libmodbus_inspector_pfn inspector)
{
...
static int send_msg(modbus_t *ctx, uint8_t *msg, int msg_length)
{
    ...
    if (ctx->inspector)
        ctx->inspector(0, msg, msg_length);
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    while (length_to_read != 0) {
        ...
        //if (_inspector)
        //    _inspector(1, msg+msg_length, rc);
    ...
    if (ctx->inspector)
        ctx->inspector(1, msg, msg_length);
                 ...
                 int req_length, modbus_mapping_t *mb_mapping)
{
    ...
    case MODBUS_FC_REPORT_SLAVE_ID: {
        ...
        str_len = 3 + (int)strlen(LIBMODBUS_VERSION_STRING);
